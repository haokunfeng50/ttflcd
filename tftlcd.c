...
#include "stdlib.h"
#include "font.h" 
#include "stm32f4xx.h"                  // Device header (includes HAL via USE_HAL_DRIVER)

// 使用CubeMX HAL的HAL_Delay替代自定义SysTick的delay_ms
...
void TFTLCD_FSMC_Init(void)
{
	...
	// Step 1: 配置BCR4控制寄存器(先不使能MBKEN)
	// 16-bit数据宽度(bit4), 写使能(bit12), 扩展模式(bit14, 读写时序分开)
	FSMC_Bank1->BTCR[6] = (1 << 4) | (1 << 12) | (1 << 14);

	// Step 2: 配置读时序(BTR4) - Mode A
	// ADDSET=15, DATAST=96
	FSMC_Bank1->BTCR[7] = (15 << 0) | (96 << 8);

	// Step 3: 配置写时序(BWTR4) - Mode A
	// ADDSET=21, DATAST=5
	FSMC_Bank1E->BWTR[3] = (21 << 0) | (5 << 8);

	// Step 4: 最后使能存储器(MBKEN)
	FSMC_Bank1->BTCR[6] |= 0x0001;
}
...
*/
	...
	LCD_WriteData(0x20);		//24λ??
	LCD_WriteData(0x00);		//TFT ?? 
...
#ifdef TFTLCD_ILI9806
		//Software Reset (fix white screen after flash)
		LCD_WriteCmd(0x01);
//		delay_ms(120);
HAL_Delay(120);
	LCD_WriteCmd(0xFF);
...
/**
  * @brief  在 LCD 指定位置显示一张图片（从数组读取像素数据）
  * @param  x     : 图片显示左上角 X 坐标（列）
  * @param  y     : 图片显示左上角 Y 坐标（行）
  * @param  wide  : 图片宽度（像素）
  * @param  high  : 图片高度（像素）
  * @param  pic   : 指向图片数据数组的指针（通常为 const u8 数组）
  * @note   图片数据格式要求：RGB565 格式，高字节在前（大端序），
  *         数据长度 = wide * high * 2 字节。
  *         该函数会自动设置 LCD 的显示窗口，然后逐点写入颜色数据。
  */
